                ...
                <div class="flex flex-wrap mx-[-15px] md:mx-[-20px] xl:mx-[-35px] mt-[-70px]">
                    ...
                    <div
                                ...
                                <div class="shopping-cart-item flex justify-between mb-4">
                                    <div class="flex flex-row items-center">
                                        <figure class="!rounded-[.4rem] !w-[7rem]">
                                            <a href="{{ route('front.product.view', $pro->slug) }}">
                                                <img class="!rounded-[.4rem]" src="{{ $photos[0] }}"
                                                    style="width:90px;height:100px;" alt="{{ $pro->title }}">
                                            </a>
                                        </figure>
                                    ...
                                    <div class="ml-2 flex items-center">
                                        <p class="price text-[0.7rem]"><span
                                                class="amount">{{ number_format($pro->price, 0, '.', ',') }}</span></p>
                                    </div>
                                </div>
                        ...
                        <div id="payment_method_detail" class="!mb-5">
                            ...
                            <p id="selected_payment_method" class="title_color font-bold !m-0">Thanh toán sau khi nhận
                                hàng
                                (COD)</p>
                        </div>
            ...
            <div class="modal-content !text-center">
                <div class="modal-body relative flex-auto pt-[2.5rem] pr-[2.5rem] pb-[2.5rem] pl-[2.5rem]">
                    ...
                    <form id="addAddressForm" class="text-left mb-3" method= "POST"
                        action="{{ route('front.profile.addshipadd') }}">
                        @csrf
                        <div class="relative mb-4">
                            <label for="email">Tên đầy đủ</label>
                            <input
                ...
                <div class="modal-body relative flex-auto pt-[2.5rem] pr-[2.5rem] pb-[2.5rem] pl-[2.5rem]">
                    ...
                    @foreach ($addressbooks as $address)
                        <div class="form-check block min-h-[1.36rem] mb-0.5" style="border-bottom:1px solid">
                            <input class="invoice_ra form-check-input" type="radio" id="ra{{ $i }}}"
                                data-name="{{ $address->full_name }}" data-phone="{{ $address->phone }}"
                                data-address=" {{ $address->address }}" class="invoice_ra" name="invoice_id"
                                value="{{ $address->id }}">
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content !text-center">
                <div class="modal-body relative flex-auto pt-[2.5rem] pr-[2.5rem] pb-[2.5rem] pl-[2.5rem]">
                    ...
                    @foreach ($addressbooks as $address)
                        <div class="form-check block min-h-[1.36rem] mb-0.5" style="border-bottom:1px solid">
                            <input class="ship_ra form-check-input" type="radio" id="ra{{ $i }}}"
                                data-name="{{ $address->full_name }}" data-phone="{{ $address->phone }}"
                                data-address=" {{ $address->address }}" class="invoice_ra" name="invoice_id"
                                value="{{ $address->id }}">
                            <label class="form-check-label" for="ra{{ $i }}}">
        ...
        `;
            ...
            $('#addAddressForm').on('submit', function(e) {
                ...
                if (payment_method === 'online') {
                    ...
                    $.ajax({
                        url: "{{ route('front.shopingcart.order') }}",
                        method: 'POST',
                        data: {
                            _token: "{{ csrf_token() }}",
