  ...
  <div class="header-wrapper">
      <div class="header-content">
          <div class="header-top">
              <div class="logo">
                <a href="/">
                  <img src="{{$setting->logo}}" alt="{{$setting->company_name}}">
                  <span></span>
                </a>
              </div>
              ...
              <div class="auth-buttons" style="display: block;">
                  <a href="{{route('front.login')}}">🔑<span class='thide'>Đăng nhập</span></a>
                  <a href="{{route('front.register')}}">✍️<span class='thide'>Đăng ký</span></a>
              </div>
              @else
              <!-- Nhóm nút khi đã đăng nhập -->
              <div class="user-actions" style="display: block;"> <!--Ẩn mặc định-->
                  <a href="{{route('front.profile')}}" class="account">👤<span class='thide'>Tài khoản</span></a>
                  <a href="{{route('front.shopingcart.view')}}" class="cart">
          ...
          <div class="menu-container">
              <nav class="menu">
                <div class="nav-item dropdown">
                  ...
                  <div class="submenu">
                    @foreach ($cats as  $cat)
                        <a class="submenu-link" href="{{route('front.product.cat',$cat->slug)}}">{{$cat->title}}</a>
                  ...
                  <div class="submenu">
                    @if (count($catblogs)> 0)
                        @foreach ($catblogs as $catblog )
                           <a class="submenu-link" href="{{route('front.category.view',$catblog->slug)}}">{{$catblog->title}}</a>
                        @endforeach
