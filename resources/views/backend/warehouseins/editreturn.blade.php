                            ...
                            <div class="mt-3">
                                ...
                                <select style="display:none" id="delivery_id" name="delivery_id" class="form-select mt-2 sm:mr-2"    >
                                    ...
                                    @foreach ($deliveries as $delivery)
                                        <option value="{{$delivery->id}}" {{$warehousein->delivery_id ==$delivery->id?'selected':''}}>{{$delivery->full_name}}</option>
...
<script src="{{asset('backend/assets/js/product_returndetail_wi.js')}}"></script> 
<script>
    $.ajaxSetup({
...
$(document).ready(function(){ //Your code here 
    $.ajax({
        type: 'GET',
        url: '{{route("warehousein.getProductListReturn")}}',
        data: {
            wi_id: {{$warehousein->id}},
            wiold_id: {{$warehouseinreturn->id}},
        ...
        success: function(data) {
            ...
            products.forEach((pitem) => {
                ...
                newpro = new Product(pitem.id,pitem.title, pitem.price,pitem.type,pitem.qty ,pitem.quantity -pitem.qty_sold + pitem.qty,imageurls[0],pitem.seri,pitem.series,plist);
...
$('#btnstore').on( "click", function() {
    ...
    $.ajax({
        url: "{{route('warehousein.updatereturndetail',$warehousein->id)}}", // Replace with your actual server endpoint URL
        method: "POST",
            ...
            if(response.status == true)
             {
                ...
                window.location.href = '{{route("warehousein.index")}}';
                return;
             }  
            else
