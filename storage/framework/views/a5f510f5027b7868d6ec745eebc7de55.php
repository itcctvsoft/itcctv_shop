...
$user = auth()->user();
if ($user) {
    $sql = 'select c.quantity, d.* from (SELECT * from shoping_carts where user_id = ' . $user->id . ") as c left join products as d on c.product_id = d.id where d.status = 'active'  ";
    ...
    <div class="wrapper !bg-[#ffffff]">
        <div class="container pt-14 xl:pt-[4.5rem] lg:pt-[4.5rem] md:pt-[4.5rem] pb-[4.5rem] xl:pb-24 lg:pb-24 md:pb-24">
            <div class="flex flex-wrap mx-[-15px] md:mx-[-20px] xl:mx-[-35px] mt-[-70px]">
                <div
                    class="xl:w-8/12 lg:w-8/12 w-full flex-[0_0_auto] xl:px-[35px] lg:px-[20px] md:px-[20px] px-[15px] mt-[70px] max-w-full">
                    ...
                    <div class="flex flex-wrap mx-[-15px] mt-0">
                        ...
                        <div
                            class="md:w-4/12 lg:w-5/12 xl:w-5/12 w-full flex-[0_0_auto] px-[15px] max-w-full ml-auto lg:!ml-0 xl:!ml-0 xl:!text-right lg:!text-right md:!text-right mt-[20px]">
                            <a href="<?php echo e(route('front.shopingcart.checkout')); ?>"
                                class="btn btn-primary text-white secondarybackgroundcolor border-[#3f78e0] hover:text-white hover:bg-[#3f78e0] hover:border-[#3f78e0] focus:shadow-[rgba(92,140,229,1)] active:text-white active:bg-[#3f78e0] active:border-[#3f78e0] disabled:text-white disabled:bg-[#3f78e0] disabled:border-[#3f78e0] rounded hover:translate-y-[-0.15rem] hover:shadow-[0_0.25rem_0.75rem_rgba(30,34,40,0.15)]">
                                Đặt hàng
                            </a>
                        </div>
                    </div>
                ...
                <div
                    class="xl:w-4/12 lg:w-4/12 w-full flex-[0_0_auto] xl:px-[35px] lg:px-[20px] md:px-[20px] px-[15px] mt-[70px] max-w-full">
    ...
    <script>
        class Productc {
            ...
            generateHTML() {
                ...
                var myhtml =
                    '<tr><td class="option text-left flex flex-row items-center !pl-0" ><figure class="!rounded-[.4rem] !w-[7rem]"><a href="/product/view/' +
                    this.slug + ' ">' +
                    '<img class="!rounded-[.4rem]" src="' +
                    this.url +
                    '" ></a></figure><div class="w-full ml-4"> <h3 class="post-title h6 !leading-[1.35] !mb-1"><a href="/product/view/' +
                    this.slug + ' " class="title_color">' +
                    ...
                    this.id + '" onchange="updateQuantity(' + this.id +
                    ')" class="ipqty py-3 px-4  text-right form-control " value="' +
                    this.quantity + '"/></div></td>' +
                    '<td><p class="price !m-0"><span class="amount">' +
                    Intl.NumberFormat().format(this.getTotalCost()) + '</span></p></td>' +
                    '<td class="!pr-0"><button onclick="removeProduct(' + this.id +
                    ')" class="title_color"><i class="uil uil-trash-alt before:content-["\ed4b"]"></i></button>' +
                    '</td></tr> ';
                return myhtml;
            }
        ...
        function update_cart(product_id, quantity) {
            ...
            $.ajax({
                url: "<?php echo e(route('front.shopingcart.update')); ?>",
                method: "POST",
                contentType: "application/json",
                data: JSON.stringify(dataToSend),
                success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            html: `
                                <div style="display: flex; flex-direction: column; align-items: center; justify-content: center;">
                                    <img src="https://i.pinimg.com/originals/91/2f/9d/912f9ddf05af991763bbfeb4beb195a2.gif" alt="Success" style="width: 150px; height: 150px; margin-bottom: 10px;">
                                </div>
                            `,
                            icon: "success",
                            timer: 3000, // Hiển thị 3 giây
                            showConfirmButton: false,
                            position: "center", // Vị trí trung tâm
                            background: "#f0f9eb", // Nền nhẹ nhàng
                        });
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    products.forEach((pitem) => {
                        ...
                        newpro = new Productc(pitem.id, pitem.title, pitem.slug, pitem.price,
                            pitem.quantity, imageurls[0]);
                        productList.push(newpro);
                    });
                    updateListView();
                },
...
<?php echo $__env->make('frontend_tp.layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\Workspace\KhoaLuan_2024\shop-main\resources\views/frontend_tp3/cart/view.blade.php ENDPATH**/ ?>