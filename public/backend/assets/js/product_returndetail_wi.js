...
class Product {
    ...
    generateHTML()
    {
        ...
        + this.name+'</a> <br/><span class="form-help px-2"> Số lượng nhập: '+ tonkho+'</span></div> </div> </td> <td class="text-right"><div class="flex"><input type="number" style="width:150px" id="ip'
        ...
        if(product.id == id)
        {
            ...
            if(product.type == 'normal')
            {
                if(ip.value*1 > product.stock_quantity*1)
                {
                    ...
                    Swal.fire(
                        'Không hợp lệ!',
                        'Số lượng lớn hơn số lượng tối đa!',
                        'error'
                    );
                    ip.value = 1;
