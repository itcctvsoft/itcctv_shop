...
class SettingController extends Controller
{
    ...
    public function updateVersion12()
    {
        \DB::beginTransaction();
        ...
        if($detail->version == 1)
        {
            ...
            //tao csdl moi tren nay

            //sau khi tao csdl thi cap nhat du lieu
            $data['detail_ins']=\DB::select(  ' select a.* , b.* from (select * from  warehouse_in_details  ) as a  left join (select id, code, supplier_id as user_id,wh_id from warehouse_ins  ) as b'
            .' on a.doc_id = b.id ');

            $data['detail_outs']=\DB::select(  ' select a.*, b.*  from (select id,wo_id as doc_id,product_id,price,quantity,created_at, prebalance,doc_type from  warehouseout_details  ) as a  left join (select id, code, customer_id as user_id,wh_id from warehouseouts   ) as b'
            .' on a.doc_id = b.id ');
            ...
            usort( $data['detail_ins'], [$this, 'compareColumntang']);
            // dd( $data['detail_ins']);
            foreach($data['detail_ins'] as $detail)
            {
                $invdetail = [];
                if($detail->quantity < 0)
                {
                    $invdetail['operation'] = -1;
                    $invdetail['quantity'] =$detail->quantity * (-1);
                }
                else
                {
                    $invdetail['operation'] = 1;
                    $invdetail['quantity'] =$detail->quantity;
                }
                $invdetail['product_id'] = $detail->product_id;
                $invdetail['price'] = $detail->price;
                $invdetail['doc_id'] = $detail->doc_id;
                $invdetail['doc_type'] = $detail->doc_type;
                $invdetail['prebalance'] = $detail->prebalance;
                $invdetail['created_at'] = $detail->created_at;
                $invdetail['updated_at'] = $detail->created_at;
                $invdetail['wh_id'] = $detail->wh_id;
                if (! $invdetail['wh_id'])
                    $invdetail['wh_id'] = 1;
                \App\Models\InventoryDetail::create($invdetail);
    ...
    public function compareColumntang($a, $b) {
